...
gain1 = readtable("../vco_div_4.csv");
gain2 = readtable("../vco_div_5.csv");
gain3 = readtable("../vco_div_6.csv");
gain4 = readtable("../vco_div_7.csv");
...
plot(gain3.Time, gain1.ControlVoltage);
hold on;
plot(gain1.Time, gain2.ControlVoltage);
hold on;
plot(gain4.Time, gain3.ControlVoltage);
hold on;
plot(gain2.Time, gain4.ControlVoltage);
grid on;
title('Loop Filter Control Voltage  vs. Divider N', 'interpreter','latex');
xlabel('Time(ns)', 'interpreter','latex');
ylabel('Voltage(V)', 'interpreter','latex');
legend('N = 4', 'N = 5', 'N = 6', 'N = 7', 'interpreter','latex')

subplot(312);

plot(gain3.Time, gain1.Frequency);
hold on;
plot(gain1.Time, gain2.Frequency);
hold on;
plot(gain4.Time, gain3.Frequency);
hold on;
plot(gain2.Time, gain4.Frequency);
grid on;
title('VCO Frequency vs. Divider N', 'interpreter','latex');
xlabel('Time(ns)', 'interpreter','latex');
...
grid on;
title('VCO Phase Noise vs. Divider N', 'interpreter','latex');
xlabel('Frequency(Hz)', 'interpreter','latex');
ylabel('Phase Noise Power(dBc/Hz)', 'interpreter','latex');
legend('Free-Running VCO', 'N = 4', 'N = 5', 'N = 6', 'N = 7', 'interpreter','latex')
