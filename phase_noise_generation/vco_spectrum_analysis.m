...
gain1 = readtable("../vco_gain_20_1.csv");
gain2 = readtable("../vco_gain_40_2.csv");
gain3 = readtable("../vco_gain_60_3.csv");
...
plot(gain1.Time, gain3.ControlVoltage);
hold on;
plot(gain2.Time, gain2.ControlVoltage);
hold on;
plot(gain3.Time, gain1.ControlVoltage);
grid on;
title('Loop Filter Control Voltage vs. $K_p:K_i$ ratios', 'interpreter','latex');
xlabel('Time(ns)', 'interpreter','latex');
ylabel('Voltage(V)', 'interpreter','latex');
legend('60:3', '40:2', '20:1', 'interpreter','latex')

subplot(312);

plot(gain1.Time, gain3.Frequency);
hold on;
plot(gain2.Time, gain2.Frequency);
hold on;
plot(gain3.Time, gain1.Frequency);
grid on;
title('VCO Frequency vs. $K_p:K_i$ ratios', 'interpreter','latex');
xlabel('Time(ns)', 'interpreter','latex');
...
grid on;
title('VCO Phase Noise vs. $K_p:K_i$ ratios', 'interpreter','latex');
xlabel('Frequency(Hz)', 'interpreter','latex');
ylabel('Phase Noise Power(dBc/Hz)', 'interpreter','latex');
legend('Free-Running VCO', '20:1', '40:2', '60:3', 'interpreter','latex')
